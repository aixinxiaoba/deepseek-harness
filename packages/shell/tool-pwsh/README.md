...
- **Language mode and named-pipe capture under the Windows sandbox** — under the [Windows ACL sandbox](../../sandbox/sandbox-windows-acl/README.md), read-only pwsh starts in ConstrainedLanguage because its temp write denial makes PowerShell's AppLocker probe fail closed: `Add-Type`, non-core .NET statics (`[System.IO.*]::`, `[math]::`), COM objects, and reflection fail with "only core types" errors, and the mode cannot be lifted from inside. Workspace-write's private temp lets the probe complete, so it stays in FullLanguage unless host policy says otherwise. Both confined modes deny named-pipe opens, so a piped-stdio spawn inside a confined command fails with EPERM. WMI/CIM is unavailable under every confined mode (`Get-CimInstance`, `Get-WmiObject`, `wmic`, `systeminfo` fail with access denied — the restricted token's SID list omits Authenticated Users, so the WMI namespace check fails with 0x80041003): the model is taught to query system facts through .NET APIs instead (e.g. `Add-Type -AssemblyName Microsoft.VisualBasic` then `Microsoft.VisualBasic.Devices.ComputerInfo`) or escalate the exact command once when WMI is genuinely required. Windows-schannel HTTPS clients (`curl.exe`, `Invoke-WebRequest`) also fail under every confined mode with SEC_E_NO_CREDENTIALS (curl exit 35) — the restricted token cannot acquire schannel credentials — while Node (OpenSSL) fetches normally, so the description points one-off HTTPS at `node -e`. The tool description teaches these contracts to the model; the backend README owns the full limitations.
