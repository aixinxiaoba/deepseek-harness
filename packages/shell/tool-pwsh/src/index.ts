...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  return base + ' Under the Windows sandbox, read-only pwsh runs in PowerShell ConstrainedLanguage mode, while '
    ...
    + 'WMI/CIM is likewise unavailable under every confined mode: `Get-CimInstance`, `Get-WmiObject`, '
    + '`wmic`, and `systeminfo` fail with access denied — a token boundary, not a command bug, so do not '
    + 'retry them another way. Query system facts through .NET APIs instead (in FullLanguage: '
    + '`Add-Type -AssemblyName Microsoft.VisualBasic`, then `New-Object Microsoft.VisualBasic.Devices.ComputerInfo` '
    + 'for memory), or escalate the exact command once as described below when WMI is genuinely required. '
    + 'HTTPS clients built on Windows schannel (`curl.exe`, `Invoke-WebRequest`) also fail under every confined '
    + 'mode with SEC_E_NO_CREDENTIALS (curl exit 35) — the restricted token cannot acquire schannel credentials. '
    + 'For one-off HTTPS from a confined command, use Node instead (OpenSSL-based and unaffected): '
    + '`node -e "fetch(\'https://…\').then(r=>r.text()).then(console.log)"`. '
