...
- **Windows 沙箱下的语言模式与 named-pipe 捕获** — 在 [Windows ACL 沙箱](../../sandbox/sandbox-windows-acl/README.zh.md) 下，read-only pwsh 会以 ConstrainedLanguage 启动，因为临时目录写入被拒绝，导致 PowerShell 的 AppLocker 探针失败并按 fail-closed 处理：`Add-Type`、非核心 .NET 静态调用（`[System.IO.*]::`、`[math]::`）、COM 对象与反射都会以“only core types”错误失败，且该模式无法从内部解除。workspace-write 的私有临时目录使探针得以完成，因此除非主机策略另有规定，否则它保持 FullLanguage。两种受限模式都拒绝 named-pipe 打开，因此受限命令内的管道 stdio spawn 以 EPERM 失败。WMI/CIM 在所有受限模式下均不可用（`Get-CimInstance`、`Get-WmiObject`、`wmic`、`systeminfo` 都报 access denied——受限令牌的 SID 列表不含 Authenticated Users，WMI 命名空间检查以 0x80041003 失败）：工具描述教模型改用 .NET API 查询系统信息（如 `Add-Type -AssemblyName Microsoft.VisualBasic` 后用 `Microsoft.VisualBasic.Devices.ComputerInfo`），或在确需 WMI 时按流程对同一条命令升级一次。Windows schannel 系 HTTPS 客户端（`curl.exe`、`Invoke-WebRequest`）在所有受限模式下同样以 SEC_E_NO_CREDENTIALS（curl 退出码 35）失败——受限令牌无法取得 schannel 凭据——而 Node（OpenSSL）不受影响，因此描述把一次性 HTTPS 指向 `node -e`。工具描述把这些约定教给模型；后端 README 负责完整的限制说明。
